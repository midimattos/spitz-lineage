...
function updateGenotypePreview() {
  ...
  el.innerHTML = `
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
      ${LOCI.map(l=>{
        const rawPair = genotype[`Locus_${l}`] || [];
        const a1 = (rawPair[0] && rawPair[0] !== '?') ? rawPair[0] : null;
        const a2 = (rawPair[1] && rawPair[1] !== '?') ? rawPair[1] : a1;
        const display = `${a1 || '—'}/${a2 || a1 || '—'}`;
        return `<div style="display:flex;align-items:center;gap:10px;padding:9px 12px;background:var(--surface2);border-radius:6px">
          ...
          <span style="font-family:monospace;color:var(--gold)">${display}</span>
        </div>`;
      }).join('')}
    </div>`;
}
